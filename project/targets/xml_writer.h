// $Id: xml_writer.h,v 1.10 2016/05/20 02:10:42 ist179620 Exp $ -*- c++ -*-
...
namespace zu {
  ...
  private:
	...
	/*ADDED*/
	inline void processString(const std::string *string, int lvl) {
      os() << std::string(lvl, ' ') + *string << std::endl;
    }

	inline void processBoolean(const bool boolean, int lvl) {
      os() << std::string(lvl, ' ') << boolean << std::endl;
    }

	inline void processBasicType(basic_type *basicType, int lvl){
		if(basicType->_name == basic_type::TYPE_UNSPEC)
			os() << std::string(lvl, ' ') << " unspec" << std::endl;
		if(basicType->_name == basic_type::TYPE_INT)
			os() << std::string(lvl, ' ') << " int" << std::endl;
		if(basicType->_name == basic_type::TYPE_DOUBLE)
			os() << std::string(lvl, ' ') << " double" << std::endl;
		if(basicType->_name == basic_type::TYPE_BOOLEAN)
			os() << std::string(lvl, ' ') << " boolean" << std::endl;
		if(basicType->_name == basic_type::TYPE_STRING)
			os() << std::string(lvl, ' ') << " string" << std::endl;
		if(basicType->_name == basic_type::TYPE_POINTER)
			os() << std::string(lvl, ' ') << " pointer" << std::endl;
		if(basicType->_name == basic_type::TYPE_STRUCT)
			os() << std::string(lvl, ' ') << " struct" << std::endl;
		if(basicType->_name == basic_type::TYPE_VOID)
			os() << std::string(lvl, ' ') << " void" << std::endl;
		if(basicType->_name == basic_type::TYPE_ERROR)
			os() << std::string(lvl, ' ') << " error" << std::endl;
		os() << std::string(lvl, ' ') << " size: " << basicType->_size  << std::endl;
	};
  ...
  public:
    //void do_program_node(zu::progconditionram_node * const node, int lvl);
